...
class PageIndicator: UIView {
  ...
  override func layoutSubviews() {
    ...
    if #available(iOS 11.0, *) {
        heightAdjust = safeAreaInsets.bottom
    } else {
        heightAdjust = layoutMargins.bottom
    }
    
    print("height Adjust = \(heightAdjust)")
    ...
    for (i, button) in buttons.enumerated() {
      ...
      button.frame = CGRect(x: width * CGFloat(i),
                            y: 0,
                            width: width,
                            height: bounds.size.height - heightAdjust)
    }

    indicator.frame.size = CGSize(width: width / 1.5, height: 4)
    indicator.frame.origin.y = bounds.size.height - heightAdjust - indicator.frame.size.height
  ...
  func setup() {
    backgroundColor = Config.PageIndicator.backgroundColor
    buttons = items.map {
